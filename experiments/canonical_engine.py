...
from .canonical_metrics import (
    ...
    paired_differences,
    posttest_metrics,
    scenario_oscillation_rate,
)
from .experiment_metrics import (
    competence_contextual_risks,
    competence_risk_weights,
)
from .synthetic_trainee import initial_scores, load_profiles


def _target(theta, definitions, target):
    return all(
        theta[c] >= target
        for c, d in definitions.items()
        if d.high_risk_related
    )
            ...
            rows.append(
                {
                    ...
                    "required_ppe": scenario.required_ppe,
                    "missing_ppe": sorted(required - selected),
                    "correct": selected == required,
                    "critical_error": (
                        critical and selected != required
                    ),
                    "critical_miss": (
                        critical and bool(required - selected)
                    ),
                }
            )
...
def run_single(
    ...
    estimate = initial_scores(
        profile,
        data["competencies"],
    )

    latent = LatentState(dict(estimate))
    initial_theta = latent.snapshot()
    ...
    risk_hat = competence_contextual_risks(data)
    weights = competence_risk_weights(data)

    # Canonical RW-UCB1 state. Counts and reward sums are observable-only.
    # Reward: r_j,t^UCB = max(0, l_j,t+1 - l_j,t).
    rw_ucb1_counts = {
        cid: 0
        for cid in estimate
    }
    ...
    sid = "S1"
    difficulty = data["scenarios"][sid].base_difficulty
    ...
    rep_seed = config.repetition_seed(
        profile_id,
        repetition,
    )
    ...
    for episode in range(1, config.episodes + 1):
        ...
        for cid in scenario.target_competencies:
            z[cid] = observation_score(
                obs["action_accuracy"][cid],
                obs["response_quality"],
                obs["independence"],
                config,
            )

            estimate[cid] = update_engine_estimate(
                estimate[cid],
                z[cid],
                config.eta,
            )
            ...
            errors[cid] = (
                0
                if obs["action_accuracy"][cid] >= 1
                else errors[cid] + 1
            )

        # Exact canonical observable learning-progress reward used by RW-UCB1.
        if method == "rw_ucb1":
            for cid in scenario.target_competencies:
                reward = max(
                    0.0,
                    estimate[cid] - before[cid],
                )
                rw_ucb1_counts[cid] += 1
                rw_ucb1_reward_sums[cid] += reward
        ...
        if method == "oracle":
            method_context = OracleContext(
                deployable_context,
                latent.snapshot(),
                risk_hat,
            )
        elif method == "rw_ucb1":
            method_context = RWUCB1Context(
                deployable=deployable_context,
                practice_counts=dict(rw_ucb1_counts),
                reward_sums=dict(rw_ucb1_reward_sums),
                episode=episode,
            )
        else:
            method_context = deployable_context
        ...
        before_theta = latent.snapshot()

        evolve_latent(
            latent,
            set(scenario.target_competencies),
            difficulty,
            config,
        )
        ...
        if reached and reached_episode is None:
        ...
        cycles.append(
            {
                ...
                "missing_ppe": sorted(required - selected),
                "correct": correct,
                "response_quality": obs["response_quality"],
                "independence": obs["independence"],
                "engine_estimate_before": before,
                "observation_scores": z,
                "engine_estimate_after": dict(estimate),
                "theta_star_before": before_theta,
                "theta_star_after": latent.snapshot(),
                "priority_by_competence": (
                    priorities["priority_by_competence"]
                ),
    ...
    run = {
        ...
        "canonical_run_id": run_id,
        "method": method,
        "simulation_only": method == "oracle",
        "profile": profile_id,
        "repetition": repetition,
        "base_seed": config.base_seed,
        "repetition_seed": rep_seed,
        "episodes_completed": len(cycles),
        "target_reached": reached_episode is not None,
        "episodes_to_target": (
            reached_episode
            or config.episodes
        ),
        "censored": reached_episode is None,
        "SOR": scenario_oscillation_rate(
            [
                cycle["scenario_id"]
                for cycle in cycles
            ]
...
def run_canonical(config, progress=None):
    ...
    for profile in config.profiles:
        for repetition in range(config.repetitions):
            for method in config.methods:
                cycle_rows, run_row, posttest_rows = run_single(
                    method,
                    profile,
                    repetition,
                    config,
    ...
    return {
        ...
        "run_rows": runs,
        "posttest_rows": post,
        "paired_rows": paired_differences(runs),
    }
