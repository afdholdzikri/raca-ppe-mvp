...
class CanonicalConfig:
    ...
    eta: float = 0.2
    w_action: float = 0.6
    w_response: float = 0.2
    w_independence: float = 0.2
    rho: float = 0.18
    kappa: float = 0.12
    practice_rate: float = 0.12
    sigma_g: float = 0.35
    latent_decay: float = 0.002
    response_assistance_coefficient: float = 0.08
    ...
    def __post_init__(self):
        if not self.methods or not set(self.methods) <= set(CANONICAL_METHODS):
            raise ValueError("invalid canonical methods")
        if not self.profiles or not set(self.profiles) <= set(PROFILES):
            raise ValueError("invalid profiles")
        if self.episodes < 1 or self.repetitions < 1 or self.posttest_repetitions < 1:
            raise ValueError("counts must be positive")
        if abs(self.w_action + self.w_response + self.w_independence - 1) > 1e-9:
            raise ValueError("observation weights must sum to one")
        if not 0 < self.eta <= 1 or self.sigma_g <= 0 or self.practice_rate < 0 or self.latent_decay < 0:
    ...
    def assistance_independence_coefficient(self):
        ...
        return round(
            (
                self.w_action * self.kappa
                + self.w_response * self.response_assistance_coefficient
            )
            / self.w_independence,
            12,
        )
    ...
    def event_seed(self, profile, repetition, episode, stream):
        raw = (
            f"v15|{self.repetition_seed(profile, repetition)}|"
            f"{episode}|{stream}"
        ).encode()
        return int.from_bytes(hashlib.sha256(raw).digest()[:8], "big") % (2**32)
...
METHOD_REGISTRY = {
    ...
    "score_adaptive": {
        "label": "Score-Adaptive",
        "simulation_only": False,
        "theta_star_access": False,
    },
    "competence_adaptive": {
        "label": "Competence-Adaptive",
        "simulation_only": False,
        "theta_star_access": False,
    },
    "rw_ucb1": {
    ...
    "oracle": {
        "label": "Oracle (privileged reference)",
        "simulation_only": True,
        "theta_star_access": True,
    },
}
...
CANONICAL_RULE_DEFINITIONS = {
    ...
    "R2": {
        "risk_gte": 0.50,
        "repeated_errors_gte": 2,
        "adaptation": "targeted_remediation",
    },
    "R3": {
        "mastery_gte": 0.80,
        "risk_gte": 0.50,
    ...
    "R4": {
        "mastery_gte": 0.65,
        "recent_correct": True,
        "adaptation": "progressive_adaptation",
    },
    "R5": {
